  ...
  reducers: {
    ...
    sortTasks: (state, action: PayloadAction<string>) => {
      if (action.payload === 'Date') {
        ...
        state.filteredTasks = state.tasks.sort((a, b) => priorityOrder[a.priority] - priorityOrder[b.priority]);
      }
    ...
    state.tasks = state.tasks.map(task => 
        reorderedTasks.find(reorderedTask => reorderedTask.id === task.id) || task
    );
